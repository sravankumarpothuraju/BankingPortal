...
describe('Read account data', () => {
  it('should read account data @router-read-account-data', () => {
    ...
    try {
      ...
      assert(accountData !== undefined, 'Has the `accountData` variable been created in `router.js`?');
      assert(accounts !== undefined, 'Has the `accounts` variable been created in `router.js`?');
    }
    assert(
