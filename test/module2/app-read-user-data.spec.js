...
describe('Read user data', () => {
  it('should read user data @router-read-user-data', () => {
    ...
    try {
      ...
      assert(userData !== undefined, 'Has the `userData` variable been created in `router.js`?');
      assert(users !== undefined, 'Has the `users` variable been created in `router.js`?');
    }
    assert(
